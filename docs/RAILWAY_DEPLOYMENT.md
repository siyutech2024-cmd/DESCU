# Railway 详细部署指南

本指南提供在 [Railway](https://railway.app/) 上部署 DESCU 后端的详细图文步骤。

## 📋 准备工作

1. **拥有 GitHub 账号**: 且已登录。
2. **代码已推送**: 确保最新的代码已推送到 GitHub 仓库 `siyutech2024-cmd/DESCU`。
3. **准备环境变量**:
   - `SUPABASE_URL`
   - `SUPABASE_SERVICE_ROLE_KEY`
   - `GEMINI_API_KEY`

---

## 🚀 方式一：通过网页控制台部署 (推荐新手)

### 1. 创建项目
1. 访问 [Railway Dashboard](https://railway.app/dashboard)。
2. 点击右上角的 **"New Project"**。
3. 选择 **"Deploy from GitHub repo"**。
4. 在弹出的列表中选择你的仓库 `siyutech2024-cmd/DESCU`。
5. 点击 **"Deploy Now"**。

### 2. 关键配置 (Root Directory)
⚠️ **非常重要**：由于后端代码在 `server` 文件夹内，必须设置根目录。

1. 项目创建后，点击卡片中的 **DESCU** 服务。
2. 进入 **"Settings"** 选项卡。
3. 向下滚动找到 **"Root Directory"** 部分。
4. 输入 `/server` 并点击保存/打钩。
5. Railway 检测到更改后会自动重新部署。

### 3. 配置启动命令
在 **"Settings"** 选项卡下的 **"Build & Deploy"** 部分，确认以下设置（通常会自动检测，但请核对）：

- **Build Command**: `npm install --include=dev && npm run build` (确保安装 TypeScript 等构建工具)
- **Start Command**: `npm start`
- **Watch Paths**: `/server/**` (可选，用于触发重新部署)

### 4. 设置环境变量
1. 切换到 **"Variables"** 选项卡。
2. 点击 **"New Variable"**，逐个添加以下变量：

| 变量名 | 值 (示例) | 说明 |
| :--- | :--- | :--- |
| `GEMINI_API_KEY` | `AIza...` | Google AI Key |
| `SUPABASE_URL` | `https://xxxx.supabase.co` | 数据库 URL |
| `SUPABASE_SERVICE_ROLE_KEY` | `eyJh...` | 后端专用密钥 |

> **注意**: 请 **不要** 手动设置 `PORT` 变量，Railway 会自动分配及注入该变量。手动设置可能导致端口不匹配。

添加完成后，Railway 会自动触发重新部署。

### 5. 生成并获取域名
1. 切换到 **"Settings"** 选项卡。
2. 找到 **"Networking"** -> **"Public Networking"**。
3. 点击 **"Generate Domain"**。
4. 复制生成的域名 (例如 `https://descu-production.up.railway.app`)。
5. **后续步骤**: 将此域名填入前端 Vercel 的环境变量 `VITE_API_URL` 中。

---

## 💻 方式二：使用 Railway CLI (极客首选)

### 1. 安装 CLI
```bash
npm i -g @railway/cli
```

### 2. 登录
```bash
railway login
# 浏览器会自动弹出并在认证后关闭
```

### 3. 初始化并链接项目
在项目根目录 (`/Users/ishak/Downloads/descu---二手智选`) 执行：

```bash
railway init
# 选择 "Empty Project" 或链接到现有项目
```

### 4. 上传与部署
```bash
# 自动识别并部署
railway up
```
如果是首次上传，CLI 可能会询问一些配置，确保指向 `server` 目录。

### 5. 设置环境变量 (命令行方式)
```bash
railway variables --set PORT=3000
railway variables --set SUPABASE_URL=https://...
# 依此类推
```

---

## 🛠 常见问题排查

### Q: 部署失败，日志显示 "Backend folder not found" 或 "package.json missing"
**A**: 99% 是因为没有设置 **Root Directory**。
请回到 Settings -> Root Directory，确保填写为 `/server`。

### Q: 部署成功，但访问 API 报 500 错误
**A**: 检查环境变量。查看 `View Logs`，如果看到 "Missing SUPABASE_URL" 等错误，说明变量未正确设置。

### Q: 构建太慢或超时
**A**: 在 Settings 中，确保 Railway 的 Plan 是 Hobby 或 Pro (如果是 Free tier 可能会有限制)，或者检查 `package-lock.json` 是否冲突。

### Q: 访问 API 报 404 错误 (x-railway-fallback)
**A**: 这是端口不匹配导致的。
如果您手动设置了 `PORT` 变量，请**删除它**。Railway 会自动分配一个动态端口（如 6234），您的应用会自动监听该端口。手动固定为 3000 往往会导致 Railway 转发失败。

### Q: 如何查看实时日志？
**A**: 点击服务卡片 -> **"Deployments"** -> 点击当前最新的一次部署 -> **"View Logs"**。
