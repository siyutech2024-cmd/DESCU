import React from 'react';
import { LogIn } from 'lucide-react';
import { useLanguage } from '@/i18n';
import { Sheet } from '@/components/ui/Sheet';
import { Button } from '@/components/ui/primitives';

interface LoginModalProps {
    isOpen: boolean;
    onClose: () => void;
    onLogin: () => void;
}

/** Google logo (inline so it works offline and in the app shell). */
const GoogleMark: React.FC = () => (
    <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
        <path fill="#FFC107" d="M43.6 20.1H42V20H24v8h11.3C33.7 32.7 29.2 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.2 8 3l5.7-5.7C34.1 6.1 29.3 4 24 4 13 4 4 13 4 24s9 20 20 20 20-9 20-20c0-1.3-.1-2.6-.4-3.9z" />
        <path fill="#FF3D00" d="M6.3 14.7l6.6 4.8C14.7 15.1 19 12 24 12c3.1 0 5.8 1.2 8 3l5.7-5.7C34.1 6.1 29.3 4 24 4 16.3 4 9.7 8.3 6.3 14.7z" />
        <path fill="#4CAF50" d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.2 35.1 26.7 36 24 36c-5.2 0-9.6-3.3-11.3-8l-6.5 5C9.5 39.6 16.2 44 24 44z" />
        <path fill="#1976D2" d="M43.6 20.1H42V20H24v8h11.3c-.8 2.2-2.2 4.2-4.1 5.6l6.2 5.2C41 35.3 44 30.1 44 24c0-1.3-.1-2.6-.4-3.9z" />
    </svg>
);

export const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onLogin }) => {
    const { t } = useLanguage();
    if (!isOpen) return null;
    return (
        <Sheet open={isOpen} onClose={onClose} size="sm" title={t('login.required')} closeLabel={t('modal.close')}>
            <div className="pt-1 pb-2 text-center">
                <div className="w-14 h-14 bg-brand-50 text-brand-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    <LogIn size={26} />
                </div>
                <p className="text-gray-600 mb-6">{t('login.prompt')}</p>
                <Button block size="lg" variant="secondary" onClick={onLogin} icon={<GoogleMark />} className="!bg-white !text-gray-900 !border-gray-200 hover:!bg-gray-50">
                    {t('login.google')}
                </Button>
                <p className="mt-5 text-xs text-gray-400">{t('login.terms')}</p>
            </div>
        </Sheet>
    );
};
